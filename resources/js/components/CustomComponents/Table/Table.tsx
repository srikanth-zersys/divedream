...
const TableContainer = <TData extends object>({
    ...
    const { getHeaderGroups, getFooterGroups, getRowModel, setPageIndex, setPageSize: setTablePageSize } = table;
    ...
    const showingEndFiltered = Math.min(currentPage * pageSize, filteredRows.length);
    ...
    return (
        <Fragment>
                            ...
                            {getHeaderGroups().map((headerGroup) => (
                                <tr key={headerGroup.id} className={thtrclassName}>
                                    {headerGroup.headers.map((header) => {
                                        ...
                                        const sortIcon = isSorted === "asc" ? "↑" : isSorted === "desc" ? "↓" : "";

                                        return (
                                            <th
                                                key={header.id}
                                                className={`${header.column.getCanSort() ? "sortable" : ""} ${thclassName}`}
                            ...
                            <tr>
                                <td colSpan={columns.length} className="p-4">
                                    <div>
                                        ...
                                        <p className="mt-2 text-center text-gray-500 dark:text-dark-500">No matching records found</p>
                                    </div>
                                </td>
                            </tr>
                        )}
                            ...
                            {getFooterGroups().map(footerGroup => (
                                <tr key={footerGroup.id}>
                                    {footerGroup.headers.map(footer => (
                                        <th key={footer.id} className="p-3 text-left border-slate-200 dark:border-zink-500">
                ...
                <div className={PaginationClassName}>
                    <div className="col-span-12 md:col-span-6">
                        <div className="text-gray-500 dark:text-dark-500">
                            Showing <b>{showingStartFiltered}</b> - <b>{showingEndFiltered}</b> of <b>{filteredRows.length}</b> Results
                        </div>
                    </div>
                    <div className="col-span-12 md:col-span-6">
                        <div className="flex justify-end pagination pagination-primary">
                            ...
                            <button
                                ...
                                className={`pagination-pre ${currentPage === 1 ? 'disabled' : ''}`}
                                disabled={currentPage === 1}
                            >
                                &lsaquo;
                            </button>
                            {Array.from({ length: totalPages }, (_, i) => i + 1).map(page => (
                                <button
                                    key={page}
                                    className={`pagination-item ${currentPage === page ? "bg-custom-50 dark:bg-custom-500/10 font-medium active" : ""}`}
            ...
            {filteredRows.length === 0 && isPaginationn && (
            ...
            {filteredRows.length > 0 && isPaginationn && totalPages === 1 && (
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4" style={{ marginTop: "20px" }}>
                    <div className="justify-self-center md:justify-self-start">
                        <div className="dt-info" aria-live="polite" id="example_info" role="status">
                            Showing {showingStartFiltered} to {showingEndFiltered} of {data.length} entries
                        </div>
                    </div>
                    <div className="md:col-start-2 justify-self-center md:justify-self-end">
                        <div className="dt-paging paging_full_numbers">
