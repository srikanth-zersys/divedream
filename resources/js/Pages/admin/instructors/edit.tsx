...
import { Instructor } from '@/types/dive-club';
...
const InstructorEdit: React.FC<Props> = ({ instructor, locations }) => {
  const { data, setData, put, processing, errors } = useForm({
    ...
    daily_rate: instructor.daily_rate?.toString() || '',
    instructor_agency: instructor.instructor_agency || 'PADI',
    instructor_number: instructor.instructor_number || '',
    instructor_level: instructor.instructor_level || '',
    instructor_cert_expiry: instructor.instructor_cert_expiry || '',
    teaching_certifications: instructor.teaching_certifications || [],
    insurance_provider: instructor.insurance_provider || '',
    insurance_policy_number: instructor.insurance_policy_number || '',
    ...
    calendar_color: instructor.calendar_color || '#3b82f6',
    status: instructor.status,
    location_ids: instructor.locations?.map((l) => l.id) || [],
    primary_location_id: instructor.locations?.find((l) => l.pivot.is_primary)?.id.toString() || '',
  });
  ...
  const handleDelete = () => {
    if (confirm('Are you sure you want to delete this instructor? This action cannot be undone.')) {
      router.delete(`/admin/instructors/${instructor.id}`);
    }
  };
  ...
  return (
    <Layout>
      <Head title={`Edit ${instructor.first_name} ${instructor.last_name}`} />
          ...
          <div className="bg-white dark:bg-gray-800 rounded-xl shadow-sm border border-gray-200 dark:border-gray-700 p-6">
            <div className="flex items-center justify-between">
              <div>
                ...
                <p className="text-sm text-gray-500 dark:text-gray-400">Current instructor status</p>
              </div>
              <select
                value={data.status}
